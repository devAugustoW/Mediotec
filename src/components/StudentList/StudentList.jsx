...
const StudentList = () => {
  ...
  const handleRegisterSubmit = async (userData) => {
    ...
    try {
      ...
      if (error.response) {
        setError(
          error.response.data.error || "Não foi possível cadastrar o aluno."
        );
      } else {
  ...
  const handleUpdateSubmit = async (userData) => {
    ...
    try {
      ...
      if (error.response) {
        setError(
          error.response.data.error || "Não foi possível atualizar o aluno."
        );
      } else {
        setError("Erro de conexão. Tente novamente mais tarde.");
      }
    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === 403) {
          ...
          setError(
            error.response.data.error || "Não foi possível deletar o aluno."
          );
        }
      } else {
        setError("Erro de conexão. Tente novamente mais tarde.");
