...
const AnnouncementForm = ({ onSubmit, onCancel, initialData, allClasses }) => {
	...
	useEffect(() => {
			if (initialData) {
					...
					setTargetClass(initialData.targetClasses && Array.isArray(initialData.targetClasses)
                ...
                <div className="custom-select">
                    ...
                    {isDropdownOpen && (
                        <div className="select-options">
                            <div
                                className={`option ${targetClass.length === allClasses.length ? 'selected' : ''}`}
                            ...
                            {allClasses.map(cls => (
                                <div
                                    key={cls._id}
                                    className={`option ${targetClass.includes(cls._id) ? 'selected' : ''}`}
                                    onClick={() => handleSelectChange(cls._id)}
                                >
                                    {cls.name}
                                </div>
                            ))}
                            <button type="button" onClick={handleOkClick} className="ok-button">OK</button>
                        </div>
                    )}
                </div>
            </div>
