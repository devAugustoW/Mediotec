...
const AnnouncementList = () => {
    ...
    const handleDeleteAnnouncement = async (announcementId) => {
        if (window.confirm('Tem certeza que deseja deletar este anúncio?')) {
            try {
                await axios.delete(`http://localhost:3000/announcements/${announcementId}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                fetchAnnouncements();
            } catch (error) {
    ...
    return (
        <div className="announcement-list-container">
            ...
            <button onClick={() => { setShowForm(true); setEditingAnnouncement(null); }}>
                Criar Comunicado
            </button>
            {showForm && (
                <AnnouncementForm
                    ...
                    <tbody>
                        {announcements.map(announcement => (
                            <tr key={announcement._id}>
                                ...
                                <td>
                                    <FaEdit onClick={() => handleEditAnnouncement(announcement)} className="action-button action-button--edit" title="Editar" />
                                    <FaTrash onClick={() => handleDeleteAnnouncement(announcement._id)} className="action-button action-button--delete" title="Deletar" />
                                </td>
                            </tr>
                        ))}
                    </tbody>
