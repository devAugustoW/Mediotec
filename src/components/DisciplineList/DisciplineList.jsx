...
const DisciplineList = () => {
  ...
  const editDiscipline = (disciplineId) => {
    const disciplineToEdit = disciplines.find(
  ...
  const handleDeleteDiscipline = async (disciplineId) => {
    try {
      const response = await axios.delete(
        `http://localhost:3000/deleteDiscipline/${disciplineId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      if (error.response) {
        setError(
          error.response.data.error || "Não foi possível deletar a disciplina."
        );
      } else {
        setError("Erro de conexão. Tente novamente mais tarde.");
      }
  ...
  return (
    <div className="discipline-list-container">
      ...
      {showForm ? (
        <form
          onSubmit={
            editingDisciplineId ? handleUpdateDiscipline : handleRegisterSubmit
          }
        ...
        >
          ...
          />
          <button type="submit">
            {editingDisciplineId ? "Atualizar" : "Cadastrar"}
          </button>
        </form>
      ) : (
        <>
